...
{
  if ((s<1)||(si<0))
    throw "отрицательный Размер или startindex";
  else 
    if (s>MAX_VECTOR_SIZE)
      throw "Размер или startindex больше, чем Max";
    else
    {
      Size=s;
      StartIndex=si;
...
{
  if (pos<StartIndex)
    throw "индекс меньше, чем StartIndex";
  else 
    if (pos>=Size+StartIndex)
      throw "индекс больше, чем размер";
    else 
